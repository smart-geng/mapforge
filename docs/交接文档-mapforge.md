...
> 最新状态校正：2026-09-11 · 对应方案 v1.63。本文主体是v1.26架构快照，**旧G8通过不是当前平滑/完整保真验收，不能单独据此接手或宣布完成**。
> 当前14条中心端域问题已进入同一二维来源预算和共享求解，23物理中心来源子项通过，不增短段；2movement及原60kmh动态未通过，无新XODR。设计工况/MAP宽度、整图/Web仍未闭合。先读[当前断点与证据](复核补充-完整中心观测与斜切端部预算-2026-09-11.md)及根目录HANDOFF，再使用本文定位历史代码。
...
| 方向 | 状态 | 入口 |
|---|---|---|
| SHP(IBD) → OpenDRIVE | 有转换主线；原速度平滑/边界联合重建尚未验收完成 | `ops/shp_to_xodr.py` |
| V2X MAP XML → OpenDRIVE | 有转换主线；来源/宽度合同及整路联合仍未完成 | `ops/map_to_xodr.py` |
