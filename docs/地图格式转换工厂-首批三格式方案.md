...
> 版本 v1.2（2026-08-13）· 基于《自动驾驶地图格式转换工厂：调研与落地方案》总体架构收敛而来，聚焦首批三格式的可执行方案。
...
| 5 | SHP → OpenDRIVE | P1 | 中心线/边线拟合参考线与宽度多项式 → laneSection 切分 → junction 重建；写出用 scenariogeneration | 拟合误差（REVIEW）；connecting road 需生成；**开源无车道级直转先例**（netconvert 仅道路级/输出 1.4，商业仅 Trian3DBuilder 有 GIS→xodr 产线）。（v1.1）IBD 逐形点曲率/坡度/超高可作拟合约束，降低误差风险 | 复杂路口人工确认 |
| 6 | MAP → OpenDRIVE | **P1**（v1.1 上调） | Node-Link-Lane → 车道对齐重建 → 连续参考线拟合 → **junction/laneLink 生成（connectsTo 直译）** → 1.5/1.6 写出 | 大量 INFERRED（定宽/直连兜底），定位为仿真可视化/场景底图级 | — |
...
**评审结论**（标杆解剖 CARLA Town03，数据见《评审-OpenDRIVE平滑度与参考线拟合路线》）：折线源可以生成平滑 OpenDRIVE。"平滑"的敌人不是段短——Town03 自身 1076 段中 line 占 66.7%、中位段长仅 0.21 m——而是**在错误层级断开**：xml2xodr 式"每两点一条独立 road"是 road 级 G0 拼接 + 折线航向跳变，必须否决。正确构成与 Town03 同构：**一条 Link/车道链 = 一条 road，planView 内多段参数几何，段间 G1（尽量 G2）连续由拟合器保证**，pred/succ 与 junction 拓扑完整。
...
- **误差控制**：最大偏差 ≤ 可配容差（默认 0.05 m，MAP 源放宽到编码分辨率量级），逐 road 记 fitting_error；超差加密分段而非放弃平滑；车道宽度默认常数、超阈值差才生成线性变宽（对齐 Town03 常态：99.6% 定宽）。
...
- **门禁**：planview 连续性检查器（相邻段端点位置差 <1 mm、航向差 <0.001 rad，精修档加曲率差）→ esmini 加载行驶烟雾测试 → ASAM QC。
